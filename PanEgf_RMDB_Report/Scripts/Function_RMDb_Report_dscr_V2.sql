...
-----------------------------------------------------------------------------------------------
--                  RMDb Report Code/ Sign Off 18/09/2024
-- Task 187297: RF463902 - Critical RMDB  Export issue CL prod -> New sign off 01/11/2024
-----------------------------------------------------------------------------------------------
...
create index perfinancial1 on perimeter_financials(entityid,statementid, financialid);
...
inner join perimeter_financials per
	  ...
	  and balances.versionid_::text= per.statementid_version::text --added 2024-10-25
where 1=1 
...
select distinct per.entityid ,per.financialid,  per.statementid ,per.statementid_version, balance.versionid_ versionid_balances ,accountid,
	  balance.sourcepopulateddate_ sourcepopulateddate_balance,sourcepopulateddate_rating,populateddate_ populateddate_balances, originrounding, originbalance
FROM olapts.facthiststmtbalancelatest balance 
inner join perimeter_financials per 
...
where per.flag='Legacy'
	  ...
	  and balance.accountid in ('1100','1520','1521','1522','1523','1640','1641','1642','1643','1646','1650','2680','2685','2686','2687',
				   '2100','2110','2115','2120','2130','2150','2400','2410','2415','2420','2430','2440','2450','2460','2470',
				   '5950','5960','3400','3405')--added 28/05/2024
;

-- If versionid_ of balances is lower than version of the statement, exclude it, as zero records are not recorded in the database

drop table if exists macros_legacy;
create temporary table macros_legacy as
select * 
from (
select * from balances_legacy 
except
select *  from balances_legacy 
where statementid_version::int > versionid_balances::int 
	)x;
...
--==========================================
--NEW FindTotalDebtService for dscr field
--==========================================

-----------------------------------FindTotalDebtService-----------------------------------

--3400+3405+2400+2410+2415+2420+2430+2460
--interest expense(-)+finance leases expences(-)+cpltd bank+cpltd other+cpltd syndicated loans+cpltd convertible+ cpltd sybordinated+finance leases (cp) 

drop table if exists totaldebtservice;
create temporary table totaldebtservice as
select entityid, statementid, financialid,sum(totaldebtservice) totaldebtservice, flag
from (
select distinct entityid,statementid,financialid, accountid,flag 	
	       ,coalesce((case when originrounding = '0' then originbalance::decimal(19,2)
						   ...
						   when originrounding = '2' then originbalance::decimal(19,2)* 100000 end),0) as totaldebtservice
from macros_all where 1=1
and accountid in ('3400','3405','2400','2410','2415','2420','2430','2460')
	)x
...
select  a.pkid_
       ...
       --====================================		
	   ,case when totaldebtservice.totaldebtservice::decimal(19,2)  = 0.00 or totaldebtservice.totaldebtservice is null then NULL 
	         else (coalesce(d.ebitda::numeric,0.00)::numeric(19,2) / totaldebtservice.totaldebtservice::decimal(19,2))::decimal(19,2)
		end as dscr
...
join olapts.factuphiststmtfinancialgift d on a.pkid_ = d.pkid_ and a.versionid_ = d.versionid_
--=====================new===============================
left join totaldebtservice totaldebtservice 
     on per.entityid = totaldebtservice.entityid 
	 and per.statementid = totaldebtservice.statementid
	 and per.financialid = totaldebtservice.financialid
--=======================================================		
...
select  a.pkid_
       ...
       --====================================		
	   ,case when totaldebtservice.totaldebtservice::decimal(19,2)  = 0.00 or totaldebtservice.totaldebtservice is null then NULL
	         else (coalesce(a.ebitda::numeric,0.00)::numeric(19,2) / totaldebtservice.totaldebtservice::decimal(19,2))::decimal(19,2)
		end as dscr		
	   ,a.statementyear::text as fnc_year
	   ,to_char(per.creditcommitteedate,'dd-MM-yyyy')as creditcommitteedate    	   	   
	   ,to_char(cast(a.statementdatekey_::varchar(15) as date),'yyyymmdd') as publish_date
...
select cdi,afm,csh,ebitda,eqty,gdwill,nt_incm,sales_revenue,netfixedassets,inventory,nettradereceivables,totalassets,commonsharecapital,tradepayable,
       ...
       currentratio,quickratio,dscr,fnc_year,creditcommitteedate,publish_date,approveddate,reference_date, entityid
...
--select * from olapts.rmdb_report(current_date)  
